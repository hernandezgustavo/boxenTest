...
class people::greglboxer {
  ...
  file { "${home}/.git-completion.sh":
    ...
    target  => "${$boxen::config::repodir}/modules/people/files/greglboxer/git-completion.sh"
  }

  file { "${home}/.git-prompt.sh":
    ensure  => link,
    target  => "${$boxen::config::repodir}/modules/people/files/greglboxer/git-prompt.sh"
  }
}
